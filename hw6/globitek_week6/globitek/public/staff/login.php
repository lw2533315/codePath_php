...
if(is_post_request() && request_is_same_domain()) {
  ...
  $remaining_locktime = throttle_time($username);
  if($remaining_locktime === 0){
  ...
  // If there were no errors, submit data to database
	if (empty($errors)) {
		...
		if(password_verify($password, $user['hashed_password'])) {
			...
			//log success, reset the account's login recorder
...
<div id="main-content">
  ...
  <?php
	if($remaining_locktime !== 0){
        echo 'Too many failed logins <br/> ';
		echo gmstrftime('%H:%M:%S',$remaining_locktime);
		echo '<br/><br/>';
	}?>
