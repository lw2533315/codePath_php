<?php
  ...
  function validate_user($user, $errors=array()) {
    ...
    if (is_blank($user['username'])) {
	 ...
	 if (is_blank($user['password'])||is_blank($user['password_confirm'])) {
      $errors[] = "Password and Password confirm cannot be blank.";
    } elseif ($user['password'] !== $user['password_confirm']) {
		 $errors[] = "Password and Password confirm cannot match.";
	}else if (!has_length($user['password'], array('min'=>12))){
		$errors[] = "Password is at least 12 characters lone  .";
	}elseif (!has_length($user['password'], array('max' => 255))) {
      $errors[] = "password must be less than 255 characters.";
    }elseif (!has_uppercase_letter($user['password'])) {
      $errors[] = "password must contain at least one uppercase letter.";
	}elseif (!has_lowercase_letter($user['password'])) {
      $errors[] = "password must contain at least one lowercase letter.";
	}elseif (!has_symbol($user['password'])) {
  ...
  //edit and new are different with the password_confirm
  //change the edit.php parameter to call validate_user
  function is_valid_password($user){
		$user2 = array(
		'id' => $user['id'],
		...
		'password_confirm' =>$user['hashed_password']
         );
		 return validate_user($user2, $errors=array());
  }
  ...
  function insert_user($user) {
    ...
    //looks we do not need to check is the column exsist or not 	
	//if(!db_query($db,"SELECT * FROM 'users' where  'hashed_password'")){  
		$sql_password = "alter table users add hashed_password varchar(1000)";
		$sql_password .= " ";
		db_query($db, $sql_password);
		
		
	// }
	 // $sql="alter table users add hashed_password varchar(255)";
	 // mysqli_query($db, $sql);
	
    $created_at = date("Y-m-d H:i:s");
    $sql = "INSERT INTO users ";
    $sql .= "(first_name, last_name, email, username,created_at, hashed_password) ";//enlarge the term of the table ?
    $sql .= "VALUES (";
  ...
  function update_user($user) {
	...
	$sql .= "hashed_password='" . password_hash(db_escape($db,$user['hashed_password']), PASSWORD_BCRYPT) . "'";
  ...
  //create new table failed_login
  function create_failed_login(){
	  global $db;
	  $sql = "CREATE TABLE failed_login ( 
  ...
  //find the usename's information 
  function find_failed_login($usename){
	  global $db;
	  $sql = "SELECT * FROM failed_login WHERE username=";
	  $sql.="'" .db_escape($db, $usename). "'";
	  $result = db_query($db,$sql);
	  return $result;
  }
  
  //insert a new usename to failed_login
  function insert_failed_login($failed_login){
	  global $db;
	  $sql = "INSERT INTO failed_login (username,counter, last_attempt) VALUES ( ";
	  $sql.="'" .db_escape($db,$failed_login['username']). "',";
	  $sql.="'" .db_escape($db,$failed_login['counter']). "',";
  ...
  //update failed_login table
  function update_failed_login($failed_login){
	  global $db;
	  $sql = "UPDATE failed_login SET ";
	  $sql .= "counter='" . db_escape($db, $failed_login['counter']) . "', ";
  ...
  //check the wrong username's wrong tried times
   function throttle_time($username) {
	...
	//username in the table, and tried time less than 10
    if($failed_login['counter'] < $threshold) {
		 $failed_login['counter'] = $failed_login['counter'] + 1;
