<?php
  ...
  function find_all_countries() {
    ...
    $country_result = db_query($db, db_escape($db,$sql));
  ...
  //code's length is less than name of state, custom //validation 3
  //code must be capital character, custom validation 4
  function validate_state($state, $errors=array()) {
    ...
    }elseif (!hase_validStates($state['name'])){
		$errors[] = "state name must be a-z A-Z";
	}
	
	  if (is_blank($state['country_id'])) {
      $errors[] = "country_id cannot be blank.";
    } elseif (!has_length($state['country_id'], array('min' => 1, 'max' => 255))) {
      $errors[] = "country_id must be between 1 and 255 characters.";
    }elseif (!is_digit($state['country_id'])){
		$errors[] = "country_id must be 0-9";
	}
	...
	elseif (!is_lessLengthToStateName($state['code'],$state['name'])){
		$errors[] = "code must be shorter than name of state.";
	}
	elseif(!is_capital($state['code'])){
	 $errors[] = "code must be capital characters";
	}
    return $errors;
  }
  ...
  function insert_state($state) {
    ...
    $sql .= "'".db_escape($db, $state['country_id'])."'";      //country id
    $sql .= ")";
  ...
  function validate_territory($territory, $errors=array()) {
    ...
    if (is_blank($territory['name'])) {
      ...
      $errors[] = "name must be between 2 and 255 characters.";
    }elseif (!hase_validStates($territory['name'])){
		$errors[] = "territory name must be a-z A-Z";
	}
		...
		}elseif(!is_digit($territory['position'])){     
		$errors[] = "position must be a int.";
	}
    return $errors;
  }
  ...
  function validate_salesperson($salesperson, $errors=array()) {
	...
	}else if (!has_validPhone($salesperson['phone'])){
			
		$errors[] = "phone number must be 0-9,spaces,(),-.";
	}
...
/* XXX@   is wrong format  custom validation1 */
	/* @ be used two times is wrong format  custom validation2
	"." must locate in the back of "@", and there is a other character between them custom validateion 5
	"." can not be the last character custom validation 6
	only one "." be in email format custom validation 7*/	
  function validate_user($user, $errors=array()) {
	...
	/* XXX@   is wrong format  custom validation1 */
	/* @ be used two times is wrong format  custom validation 2*/
    ...
    } elseif (!has_validSignal($user['username'])){
	  $errors [] = "Username must be a-z,A-Z,0-9, _ .";
	}
